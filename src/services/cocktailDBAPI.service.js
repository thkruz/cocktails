export const fetchCocktailEnrichment = async cocktail => {
  const cocktailName = cocktail.name;

  const response = await fetch(
    `https://www.thecocktaildb.com/api/json/v1/1/search.php?s=${cocktail.searchTerm || cocktailName}`
  );
  ...
  const variants = drinks
    ...
    .map(d => {
      ...
      return {
        ...
        ingredients: ingredients.filter(() => true),
      };
    });
