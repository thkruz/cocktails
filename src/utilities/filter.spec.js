...
describe('makeableFrom rule', () => {
  it('returns cocktails makeable from the passed-in ingredients', () => {
    const filter = {
      rule: 'makeableFrom',
      ingredients: ['Gin', 'Italicus', 'Lillet Blonde'],
    };

    const results = applyFilter(cocktails, filter);
    expect(results.length).toEqual(1);
    expect(results[0]).toHaveProperty('name', 'Hail Caesar');
  });

  it('allows for rogue ingredients', () => {
    const filter = {
      rule: 'makeableFrom',
      ingredients: ['Gin', 'Italicus', 'Lillet Blonde', 'Sugar', 'Somethign else'],
    };
...
describe('mustInclude rule', () => {
  ...
  it('does not return cocktails if a rogue ingredient exists', () => {
    const filter = {
      rule: 'mustInclude',
      ingredients: ['Gin', 'Cherry liqueur', 'Orange Bitters', 'Lemon juice', 'Foobar'],
    };
    const results = applyFilter(cocktails, filter);
    expect(results.length).toEqual(0);
  });
...
describe('combining filters', () => {
  it('allows you to combine filters', async () => {
    ...
    const filter1 = {
      rule: 'makeableFrom',
      ingredients: ['Italicus', 'Gin', 'Lillet Blonde'],
    };
    expect(applyFilter(cocktails, filter1).length).toEqual(1);
    ...
    expect(applyFilter(cocktails, filter2).length).toEqual(1);
    ...
    expect(combinedResults.length).toEqual(1);
  });
});
